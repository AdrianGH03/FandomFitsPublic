...
const womensMainPage = () => {
  const router = useRouter();
  const [clothingList, setClothingList] = useState(list);
  const [filterWord, setFilterWord] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 21; 
  const womensList = list.filter(item => item.topCat === 'womens');
  ...
  useEffect(() => {
    setClothingList(womensList);
  }, [])
  useEffect(() => {
    applyFilter(filterWord, womensList, setClothingList);
      ...
      <ul>
        ...
        <li>
          ...
          <ul className="dropdown-menu">
              ...
              <li><Link href="#" onClick={() => setFilterWord('pants')}>Pants</Link></li>
              <li><Link href="#" onClick={() => setFilterWord('leggings')}>Leggings</Link></li>
